...
function TableContainer({ loading, data, meta, handlePageChange, onDelete, handleUpdateClick, handleCreateClick, handleFilters }) {
    ...
    const columns = [
        ...
        },
        {
            title: 'VEÍCULO',
            dataIndex: 'vehicle',
            render: (vehicle) => vehicle?.title,
        },
        {
            title: 'DATA',
            dataIndex: 'pickup_date',
            render: (date, row) => dayjs(date).format('DD-MM-YYYY') + " até " + dayjs(row.return_date).format('DD-MM-YYYY')
        },
        {
            title: 'PREÇO',
            dataIndex: 'price',
        ...
        },
        {
            title: '',
            dataIndex: 'id',
            render: (id) => <Link to={"/painel/overlands/" + id}>ver detalhes</Link>
        },
